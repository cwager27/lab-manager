...
const CATEGORY_COLORS = [
  ...
  '#1ABC9C', '#9B59B6', '#3498DB', '#E67E22', '#95A5A6'
];
      ...
      <div style={{ display: 'flex', gap: '16px', marginBottom: '12px', flexWrap: 'wrap' }}>
        ...
        {grant.end_date && (
          <div>
            ...
            <p style={{ fontSize: '14px', fontWeight: 600, color: isExpiringUrgent ? '#E74C3C' : 'var(--text-primary)', margin: 0 }}>{grant.end_date}</p>
          </div>
        )}
      </div>
      {pct !== null && (
...
export default function Finance({ userRole }) {
  ...
  const [newOrder, setNewOrder] = useState({
    ...
    unit_price: '', units: '', order_date: '', requestor: '', status: 'pending', notes: ''
  });
  ...
  async function fetchData() {
    setLoading(true);
    const [{ data: grantData }, { data: orderData }, { data: reagentData }] = await Promise.all([
  ...
  async function handleConfirmImport() {
    ...
    const res = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/import-orders`, {
      method: 'POST',
  ...
  const grantSpend = orders.reduce((acc, o) => {
    ...
    acc[o.grant_name] = (acc[o.grant_name] || 0) + o.total_price;
    return acc;
  }, {});
  const grantChartData = Object.entries(grantSpend)
  ...
  const filteredOrders = orders.filter(o =>
    ...
    o.requisition_id?.toLowerCase().includes(searchQuery.toLowerCase())
  );
      ...
      {alertGrants.length > 0 && (
        <div style={{ background: '#FEF9E7', border: '1px solid #FAD7A0', borderRadius: 'var(--radius-md)', padding: '14px 16px', marginBottom: '20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
            ...
            <span style={{ fontSize: '13px', fontWeight: 600, color: '#F39C12' }}>Grant Alerts</span>
          </div>
          {alertGrants.map(g => {
            const pct = g.total_amount && g.remaining_balance ? (g.remaining_balance / g.total_amount) * 100 : null;
            const daysLeft = g.end_date ? Math.ceil((new Date(g.end_date) - new Date()) / (1000 * 60 * 60 * 24)) : null;
            return (
              <p key={g.id} style={{ fontSize: '12px', color: '#F39C12', margin: '4px 0 0' }}>
        ...
        <div style={{ background: '#EAF7F0', border: '1px solid #A9DFBF', borderRadius: 'var(--radius-md)', padding: '16px', marginBottom: '16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ fontSize: '13px', fontWeight: 600, color: '#27AE60' }}>{previewData.length} new orders found</span>
            </div>
            ...
            <>
              ...
              <div style={{ background: 'var(--bg-primary)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', overflow: 'auto' }}>
                ...
                {filteredOrders.length > 100 && (
                  <p style={{ padding: '12px 16px', fontSize: '12px', color: 'var(--text-muted)', borderTop: '1px solid var(--border)', margin: 0 }}>
                    Showing 100 of {filteredOrders.length} orders. Use search to filter.
                  </p>
                )}
              </div>
            </>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '16px' }}>
              {[
                ...
                <div key={field.key} style={{ gridColumn: field.full ? '1 / -1' : 'auto' }}>
                  ...
                  <input type={field.type || 'text'} value={newOrder[field.key]}
              ...
              <div>
                ...
                <select value={newOrder.status} onChange={e => setNewOrder(p => ({ ...p, status: e.target.value }))}
