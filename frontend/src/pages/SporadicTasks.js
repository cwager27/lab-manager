...
import {
  ...
  Calendar, User, AlertTriangle, Upload,
  Send, Filter
} from 'lucide-react';
...
const STATUS_STYLES = {
  pending: { bg: '#F5EEF8', text: '#7B3FA0', label: 'Pending' },
...
export default function SporadicTasks({ userRole, userId, profile }) {
  ...
  async function fetchData() {
    ...
    let query = supabase
      ...
      .select(`
        *,
        assignee:profiles!sporadic_tasks_assigned_to_fkey(full_name, email),
        assigner:profiles!sporadic_tasks_assigned_by_fkey(full_name)
        `)
      .order('due_date');
  ...
  function isOverdue(dueDate, status) {
    return status !== 'submitted' && dueDate < new Date().toISOString().split('T')[0];
  }
  ...
  async function handleCreateTask(e) {
    ...
    const { data, error } = await supabase
      .from('sporadic_tasks')
      .insert([{ ...newTask, assigned_by: userId, status: 'pending' }])
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '13px', marginTop: '2px' }}>One-off assignments tracked per lab member</p>
        </div>
        {canManage && (
      ...
      <div style={{ display: 'flex', gap: '12px', marginBottom: '24px' }}>
        {[
          ...
          <div key={stat.label} style={{
            flex: 1, background: stat.bg, borderRadius: 'var(--radius-md)',
        ...
        filteredTasks.map(task => {
          ...
          return (
            <div key={task.id} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px', marginBottom: '12px' }}>
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px', flexWrap: 'wrap' }}>
                    ...
                    <span style={{ padding: '2px 8px', borderRadius: '12px', fontSize: '11px', fontWeight: 600, background: catColors.bg, color: catColors.text, border: `1px solid ${catColors.border}` }}>
                      {task.category}
                    </span>
                  ...
                  {task.description && (
                    <p style={{ fontSize: '13px', color: 'var(--text-secondary)', margin: '0 0 8px' }}>{task.description}</p>
                  )}
                  <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
                    ...
                    {task.assignee && (
                      <span style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '12px', color: 'var(--text-muted)' }}>
                        <User size={12} /> {task.assignee.full_name}
                      </span>
                    )}
                    {task.assigner && (
                      <span style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '12px', color: 'var(--text-muted)' }}>
                        <Clock size={12} /> Assigned by {task.assigner.full_name}
                      </span>
                    )}
                  </div>
                </div>
              ...
              {/* Response controls */}
              {isAssignedToMe && task.status !== 'submitted' && (
                <div style={{ borderTop: '1px solid var(--border)', paddingTop: '12px', display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                  {task.response_type === 'yes_no' || task.response_type === 'yes_no_na' ? (
                    <>
                      <button onClick={() => handleResponse(task.id, 'yes')} style={{
                      ...
                      }}><CheckCircle size={14} /> Yes</button>
                      <button onClick={() => handleResponse(task.id, 'no')} style={{
                        display: 'flex', alignItems: 'center', gap: '6px', padding: '6px 14px',
                        borderRadius: 'var(--radius-md)', border: '2px solid',
                        ...
                        fontSize: '13px', fontWeight: 500
                      }}><XCircle size={14} /> No</button>
                      {task.response_type === 'yes_no_na' && (
                        <button onClick={() => handleResponse(task.id, 'na')} style={{
                          padding: '6px 14px', borderRadius: 'var(--radius-md)', border: '2px solid',
                    ...
                    <button onClick={() => handleResponse(task.id, response?.response === 'checked' ? '' : 'checked')} style={{
                      display: 'flex', alignItems: 'center', gap: '6px', padding: '6px 14px',
                      borderRadius: 'var(--radius-md)', border: '2px solid',
                  ...
                  {response?.response && (
                    <button onClick={() => handleSubmitTask(task)} style={{
                      display: 'flex', alignItems: 'center', gap: '6px', padding: '6px 14px',
              ...
              {task.status === 'submitted' && (
                <div style={{ borderTop: '1px solid var(--border)', paddingTop: '12px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                  ...
                  <span style={{ fontSize: '12px', color: 'var(--success)', fontWeight: 500 }}>
                    Completed — Response: {task.response}
                  </span>
                </div>
              )}
            </div>
      ...
      {showNewTaskForm && canManage && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }}>
          <div style={{ background: 'var(--bg-primary)', borderRadius: 'var(--radius-lg)', padding: '32px', width: '520px', boxShadow: 'var(--shadow-lg)', border: '1px solid var(--border)' }}>
            <h2 style={{ fontSize: '18px', fontWeight: 700, marginBottom: '20px' }}>Create Sporadic Task</h2>
            ...
            <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
              ...
              <button onClick={handleCreateTask} disabled={!newTask.title || !newTask.assigned_to || !newTask.due_date} style={{
                padding: '10px 20px', borderRadius: 'var(--radius-md)', border: 'none',
                background: !newTask.title || !newTask.assigned_to || !newTask.due_date ? 'var(--border)' : 'var(--purple-primary)',
                color: !newTask.title || !newTask.assigned_to || !newTask.due_date ? 'var(--text-muted)' : 'white',
                fontWeight: 600
