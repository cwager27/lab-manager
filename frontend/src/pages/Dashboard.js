...
export default function Dashboard({ profile, userRole, userId }) {
  ...
  async function fetchAll() {
    ...
    const [{ data: vacData }, { data: labData }, { data: adhocData }, { data: sporData }, { data: assignData }, { data: contactData }, { data: memberData }, { data: taskDefData }] = await Promise.all([
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <div style={{ maxWidth: '820px', display: 'flex', flexDirection: 'column', gap: '14px' }}>

          {/* ── Row 1: Time away + Meetings ── */}
                    ...
                    return (
                      <div key={r.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0 }}>
                          <span style={{ padding: '2px 6px', borderRadius: '10px', fontSize: '10px', fontWeight: 600, background: color.bg, color: color.text }}>
                            {r.leave_type}
                          </span>
            ...
            {upcoming.length > 0 && (
              <div style={{ flex: '1 1 200px', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', overflow: 'hidden', boxShadow: 'var(--shadow-sm)' }}>
                <div style={{ padding: '10px 14px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: '7px' }}>
                  ...
                  <span style={{ fontSize: '12px', fontWeight: 600, color: 'var(--text-primary)' }}>Upcoming Time Away</span>
                </div>
                <div style={{ padding: '8px 14px', display: 'flex', flexDirection: 'column', gap: '7px' }}>
                  {upcoming.map(r => {
                    const color = leaveColor(r.leave_type);
          ...
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', overflow: 'hidden', boxShadow: 'var(--shadow-sm)' }}>
            ...
            <div style={{ padding: '10px 14px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px', flexWrap: 'wrap' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '7px' }}>
                ...
                <span style={{ fontSize: '12px', fontWeight: 600, color: 'var(--text-primary)' }}>{canEdit ? 'Tasks' : 'My Tasks'}</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
                {availableFreqs.length > 0 && availableFreqs.map(f => {
                ...
                {assignType === 'oneoff' ? (
                  <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', alignItems: 'center' }}>
                    ...
                    <button
                      ...
                      disabled={saving || !assignForm.title.trim() || !assignForm.assignedTo || !assignForm.dueDate}
                      style={{ padding: '5px 11px', background: 'var(--purple-primary)', color: 'white', border: 'none', borderRadius: 'var(--radius-sm)', fontSize: '12px', cursor: 'pointer', fontWeight: 600, opacity: (saving || !assignForm.title.trim() || !assignForm.assignedTo || !assignForm.dueDate) ? 0.5 : 1 }}
                    >Save</button>
                    <button onClick={() => setShowAssignForm(false)} style={{ padding: '5px 8px', background: 'none', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', fontSize: '12px', cursor: 'pointer', color: 'var(--text-muted)', display: 'flex', alignItems: 'center' }}>
                      <X size={12} />
                    </button>
                  </div>
                ) : (
                  <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', alignItems: 'center' }}>
                    <select
                      value={recurringForm.taskId}
                      onChange={e => setRecurringForm(p => ({ ...p, taskId: e.target.value }))}
                    ...
                    >
                      ...
                      {taskDefs.map(t => (
                        <option key={t.id} value={t.id}>{t.title} ({t.frequency})</option>
                      ))}
                    </select>
                    <select
                      value={recurringForm.assignedTo}
                      onChange={e => setRecurringForm(p => ({ ...p, assignedTo: e.target.value }))}
                      style={{ flex: '1 1 130px', padding: '5px 8px', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', fontSize: '12px', background: 'var(--bg-card)', color: 'var(--text-primary)' }}
                    >
                      <option value="">Assign to…</option>
                      {members.map(m => <option key={m.id} value={m.id}>{m.full_name}</option>)}
                    ...
                    <button
                      ...
                      disabled={saving || !recurringForm.taskId || !recurringForm.assignedTo}
                      style={{ padding: '5px 11px', background: 'var(--purple-primary)', color: 'white', border: 'none', borderRadius: 'var(--radius-sm)', fontSize: '12px', cursor: 'pointer', fontWeight: 600, opacity: (saving || !recurringForm.taskId || !recurringForm.assignedTo) ? 0.5 : 1 }}
                    >Assign</button>
                    <button onClick={() => setShowAssignForm(false)} style={{ padding: '5px 8px', background: 'none', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', fontSize: '12px', cursor: 'pointer', color: 'var(--text-muted)', display: 'flex', alignItems: 'center' }}>
                      <X size={12} />
                    </button>
            ...
            <div style={{ padding: '6px 14px', display: 'flex', flexDirection: 'column', gap: '1px' }}>
                  ...
                  <div key={task.id} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '5px 0', borderBottom: '1px solid var(--border)' }}>
                    {isEditing ? (
                      <>
                        ...
                        />
                        <button onClick={() => handleSaveEdit(task.id)} disabled={saving} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#27AE60', padding: '2px', display: 'flex' }}>
                          <Check size={13} />
                        </button>
                        <button onClick={() => setEditingTask(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', padding: '2px', display: 'flex' }}>
                          <X size={13} />
                        </button>
                      </>
                    ) : (
                      <>
                        ...
                        {canEdit && (
                          <>
                            <button
                              onClick={() => { setEditingTask(task.id); setEditForm({ title: task.title, assignedTo: task.assigned_to, dueDate: task.due_date }); setShowAssignForm(false); }}
              ...
              {/* Divider if both sporadic and recurring */}
                      ...
                      <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '5px 0' }}>
                        ...
                        {isReassigning ? (
                          <>
                            ...
                            <button onClick={() => handleReassign(a.id)} disabled={saving} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#27AE60', padding: '2px', display: 'flex', flexShrink: 0 }}>
                              <Check size={13} />
                            </button>
                            <button onClick={() => setEditingAssignment(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', padding: '2px', display: 'flex', flexShrink: 0 }}>
                              <X size={13} />
                            </button>
                          </>
                        ) : (
                          <>
                            ...
                            {canEdit && (
                              <button
                                onClick={() => { setEditingAssignment(a.id); setReassignTo(a.assigned_to); setEditingTask(null); setShowAssignForm(false); }}
                                style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', padding: '2px', display: 'flex', flexShrink: 0 }}
                              >
                                <Pencil size={11} />
                              </button>
          ...
          {contacts.length > 0 && (
            <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', overflow: 'hidden', boxShadow: 'var(--shadow-sm)' }}>
              <div style={{ padding: '10px 14px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: '7px' }}>
                <Users size={13} color="var(--purple-primary)" />
                <span style={{ fontSize: '12px', fontWeight: 600, color: 'var(--text-primary)' }}>Lab Directory</span>
              </div>
                  ...
                  return (
                    <div key={contact.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 14px', borderTop: i > 0 ? '1px solid var(--border)' : 'none' }}>
                      <div style={{ width: '28px', height: '28px', borderRadius: '50%', background: colors.bg, border: `1px solid ${colors.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                        <span style={{ fontSize: '12px', fontWeight: 700, color: colors.text }}>{initial}</span>
                      </div>
