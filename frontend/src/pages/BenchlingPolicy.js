const s = {
  ...
  h3: { fontSize: 13, fontStyle: 'italic', fontWeight: 700, color: 'var(--text-primary)', margin: '16px 0 6px' },
  ...
  return (
    <div>
      ...
      <div style={s.section}>
        ...
        <p style={s.p}><strong>Definition:</strong> Any <strong>conclusive</strong> positive <em>and</em> negative results that (i) will ever lead to or directly be used in publications, presentations, grants, or patents; or (ii) advance, focus, or formulate existing or new biological and/or technical hypotheses or scientific directions.</p>
        ...
        <ul style={s.ul}>
          ...
          <li style={s.li}><u>Lab notebook pages:</u> Cross-reference lab notebook information (book ID and page numbers, e.g. SWA #1: pg 1–2). Entered dates must match the dates recorded in the marked lab notebook entries.</li>
          ...
          <li style={s.li}><u>Raw data:</u> Paths to raw data files on server OR uploads of raw data/measurements from instruments used.
        ...
        <p style={s.p}><strong>Definition:</strong> <strong>Conclusive (positive AND negative) technical results that may not be published/pursued immediately</strong> but are relevant technical knowledge for the lab (e.g. experiments testing/optimizing various protocols/reagents/assays). Require less detailed Benchling recording (but standard lab notebook recording) using the following sections:</p>
        <ul style={s.ul}>
          <li style={s.li}><u>Date</u></li>
          <li style={s.li}><u>Lab notebook pages:</u> Cross-reference lab notebook information (ID and page numbers)</li>
          <li style={s.li}><u>Category:</u> qPCR, microscopy, western, live cell imaging, sequencing, tissue isolation, etc.</li>
          <li style={s.li}><u>Experimental purpose:</u> A brief statement of experiment purpose/hypothesis.</li>
          <li style={s.li}><u>Results and Conclusions:</u> A brief statement of results/conclusions (i.e. reagents/concentrations/conditions that did not work, worked less well, or worked well but were not pursued and why).</li>
        </ul>
